...
reencode_video() {
    ...
    local PRESET=4
    local FILM_GRAIN=0
    local SVT_AV1_TUNE=0 # Force objective metrics tuning for correct analysis
    local PIX_FMT="yuv420p10le"
    local CLIP_LENGTH="30.0"

    # CRITICAL FIX: Reset array explicitly on every function call
    local -a FFMPEG_ARGS
    ...
    while [[ "$OPTIMIZED" == "false" ]]; do
        ...
        # Validate structural timeline indexing alignment
        local CND_FRAMES
        ...
        if [[ $ssim_passes -eq 1 ]] && [[ $vmaf_passes -eq 1 ]]; then
            # Check if massive visual headroom exists to optimize CPU time
            ...
            # Quality step execution down to target floor boundaries
            if [[ "$CRF" -le 18 ]]; then
                printf "WARNING: Hit minimum quality boundary limit (CRF 18). Proceeding with forced configuration.\n" >&2
                OPTIMIZED=true
            else
                CRF=$((CRF - 2))
                PRESET=4 # Revert back to deeper analysis speed to recover metrics loss
            fi
        fi
    done
...
get_video_info() {
    ...
    # --- CODEC WHITELIST FILTER ---
    case "${CODEC,,}" in
        av1|hevc|vp9)
            printf "SKIP: '%s' is already utilizing a highly efficient modern codec (%s). Re-encoding skipped.\n" "$VIDEO_FILE" "$CODEC"
            return 0
            ;;
        *)
            # Proceed with processing legacy codecs (h264, mpeg4, vc1, mpeg2video, etc.)
            print_verbose "Codec '%s' requires modernization processing." "$CODEC"
            ;;
    esac
    # --- END CODEC WHITELIST FILTER ---
...
main() {
    ...
    while IFS= read -r -d '' SRC_FILE; do
        # 1. Path Collision & Idempotency Filter
        local TARGET_MKV="${SRC_FILE%.*}.mkv"

        if [[ -f "$TARGET_MKV" ]]; then
            # Scenario A: The file is already an .mkv file
            if [[ "$SRC_FILE" == "$TARGET_MKV" ]]; then
                # Safe to proceed; the Codec Whitelist Filter inside get_video_info
                # will catch it if it's already AV1/HEVC/VP9.
                print_verbose "Evaluating existing Matroska container: '$SRC_FILE'"
            else
                # Scenario B: A legacy file (.avi/.mp4) has a matching .mkv clone on disk
                printf "SKIP: Target master asset '%s' already exists. Assuming prior processing.\n" "$TARGET_MKV"
                continue
            fi
        fi

        # 2. Standard Mime/Stream Validation Filters
